...
TEXT = ("Instrumental underground mental acidcore and tribe tekno, around 163 BPM. "
        "After the vocal hook the track builds with rising energy into an intense drive: "
        "pounding kick drum, distorted rolling bass, screeching acid lead, rave stabs, "
        "rising tension, high energy, no vocals.")
SR_MODEL = 32000
SR_OUT = 44100
FADE_MS = 2.0
...
def arc(p, sr=SR_OUT):
    """Energy build check: second-half vs first-half RMS. >1.05 = rising (hype)."""
    import librosa as _l
    r = _l.feature.rms(y=p, frame_length=2048, hop_length=512)[0]
    h = len(r) // 2
    return {"rms_1st": round(float(r[:h].mean()), 4),
            "rms_2nd": round(float(r[h:].mean()), 4),
            "slope": round(float(r[h:].mean() / max(r[:h].mean(), 1e-6)), 3)}
...
def main():
    ...
    manifest = {
        ...
        "new_section_arc": arc(gen441),
        "context_arc": arc(ctx_mono),
