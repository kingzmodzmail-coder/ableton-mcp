...
for b in range(BEATS):
    ...
    place(kick, sin_k, float(b), g)

out_path = OUT / "e_kick_sinkick_only_155.wav"
...
idx = by["E-Kick"]
path = str(out_path)
for row in [0, 2, 3, 4, 6, 7]:
    send("delete_clip", {"track_index": idx, "clip_index": row})
    ok(send("create_audio_clip", {"track_index": idx, "clip_index": row, "path": path}), f"row{row}")
    send("set_clip_name", {"track_index": idx, "clip_index": row, "name": "sinkick_only"})
