# -*- coding: utf-8 -*-
"""EvAIx MentalClean + Molecular MORPH @162 FineTuniX (FTX).

Keep MentalClean kick/bass @162, Molecular story morphs, RingMod lead, hat choke,
Valve Force mild grit, poly acid 13/15. NEW bounce (does not overwrite v1/v2).

FineTuniX polish (apply order):
1) Master limiter ceiling -1.0 to -1.5 dBTP (no TP > -1.0)
2) Rebuild Mental Tribe / 303 acid mid presence ~700 Hz-3 kHz + thin air >6 kHz
3) 1-2 early chapters in first ~70s + open macrodynamics toward LRA ~4-7 via
   filter/layer automation (NOT more master squash)
4) Kick/bass mono; optional width only on acid/FX
5) Free-party target -8 to -7 LUFS integrated if clean after peaks fixed
"""
...
OUT = BRIDGE / "samples" / "crisp-dna" / "mentalclean-molecular-morph-162-ftx"
...
MP3 = Path(r"C:\Users\Gebruiker\Downloads\EvAIx_MentalClean_MolecularMorph_162_FTX.mp3")
NOTES = OUT / "mentalclean_molecular_morph_162_ftx_notes.md"

# Full Mix Learner map (MP3)
FULL_SECTIONS = [
    # First ~71s: two extra FineTuniX chapters so intro is not one flat plateau
    ("intro", 12, "intro"),                 # 0–17.8s sparse veiled
    ("intro_tribe", 12, "intro_tribe"),     # 17.8–35.6s tribe fill + acid tease + air
    ("predrop_vocal", 12, "predrop"),       # 35.6–53.3s Pozek mid chops
    ("predrop_rise", 12, "predrop_rise"),   # 53.3–71.1s acid accent + hats + filter open
    ("drop_1", 32, "drop"),
    ("break_1", 16, "break"),
    ("build", 24, "build"),
...
ABLETON_SCENES = [
    (0, "Intro_Tribe", "intro_tribe", 8),
    (1, "Predrop_Rise", "predrop_rise", 8),
    (2, "Drop_1", "drop", 8),
    (3, "Break_1", "break", 8),
    (4, "Build", "build", 8),
...
OH_G = 0.11
PERC_G = 0.20
ACID_G = 0.30  # FTX: mid-presence 303
STRETCH_G = 0.26
...
# FineTuniX: optional width on acid/FX only (kick/bass stay mono)
ACID_WIDTH = 0.18   # Haas-ish side amount 0..1
ACID_HAAS_MS = 7.5
AIR_NOISE_G = 0.045  # thin air >6 kHz layer
ACID_HPF_HZ = 160.0
ACID_PRESENCE_HZ = 2800.0
ACID_PRESENCE_DB = 3.2


# CRISP acoustic: light glue, presence ~6.5k, NOT dense smash
# FTX master: ceiling -1.5 dBTP first (linear 10**(-1.5/20)≈0.841), then
# free-party loudnorm toward -7.5 LUFS with TP=-1.5 (encode-safe ≤ -1.0).
# LRA target via arrangement — do NOT crush with harder master squash.
AF_CHAIN = (
    "highpass=f=28:poles=2,"
    "lowshelf=f=85:t=q:w=0.7:g=1.4,"
    "equalizer=f=52:t=q:w=1.0:g=3.0,"
    "equalizer=f=75:t=q:w=1.3:g=-1.8,"
    "equalizer=f=260:t=q:w=1.0:g=-1.6,"
    "equalizer=f=380:t=q:w=1.0:g=-1.3,"
    "equalizer=f=900:t=q:w=1.0:g=1.4,"
    "equalizer=f=1800:t=q:w=1.0:g=1.6,"
    "equalizer=f=2800:t=q:w=1.2:g=2.0,"
    "equalizer=f=6500:t=q:w=1.1:g=2.0,"
    "equalizer=f=10000:t=q:w=1.1:g=-1.4,"
    "highshelf=f=8000:t=q:w=0.7:g=-1.6,"
    "lowpass=f=15500:poles=1,"
    "acompressor=threshold=-18dB:ratio=1.30:attack=12:release=180:makeup=1.06,"
    "loudnorm=I=-7.5:TP=-1.5:LRA=7,"
    "volume=-0.5dB,"
    "alimiter=limit=0.841:attack=1:release=50:level=disabled"
)

classic.RES_Q = 0.22  # FTX: ~80% res feel
classic.BASE_CUT0 = 480.0  # start ~20% open into mids
classic.ENV_AMT = 3200.0  # +70% amount into 700Hz-3k
classic.ROLL_MAX = 120.0
classic.GRIT = 1.55  # light sat
...
def peaking_eq(samples, freq_hz: float, gain_db: float, q: float = 1.0):
    """Simple peaking EQ via parallel bandpass blend (presence boost)."""
    if abs(gain_db) < 0.05:
        return list(samples)
    g = 10 ** (gain_db / 20.0) - 1.0
    bw = max(80.0, freq_hz / max(0.3, q))
    bp = bandpass(samples, max(40.0, freq_hz - bw), freq_hz + bw)
    return [s + g * b for s, b in zip(samples, bp)]


def thin_air_noise(n: int, seed: int = 42) -> list[float]:
    """Thin hissy air layer, heavily HPF'd >6 kHz."""
    rng = dna.mulberry32(seed + 6060)
    out = [(rng() * 2.0 - 1.0) * 0.35 for _ in range(n)]
    out = one_pole(out, 6200.0, mode="hpf", q=0.08)
    out = one_pole(out, 7000.0, mode="hpf", q=0.1)
    out = one_pole(out, 14000.0, mode="lpf", q=0.12)
    for i in range(n):
        t = i / SR
        out[i] *= 0.55 + 0.45 * abs(math.sin(2.0 * math.pi * 5.3 * t))
    return out


def haas_widen(mono, width: float = ACID_WIDTH, delay_ms: float = ACID_HAAS_MS):
    """Return (L, R) with mono center; width only via delayed side (acid/FX)."""
    n = len(mono)
    if width <= 0.001:
    ...
    for i in range(n):
        m = mono[i]
        delayed = mono[i - d] if i >= d else 0.0
        left[i] = m + delayed * width * 0.55
        right[i] = m - delayed * width * 0.55
    return left, right
...
def mode_kit(mode: str):
    ...
    kits = {
        ...
        "intro_tribe": dict(
            kick_mul=0.62, bass_mul=0.40, hats_mul=0.28, oh_mul=0.12, perc_mul=0.22,
            stretch_mul=0.48, acid_mul=0.28, vox_mul=0.40, click_mul=0.15, valve=0.72, air=0.35,
            hat_c="hh5c", hat_o="hh5o", rim="rim2", perc="synperc", stretch="177_PercLP-3.wav",
            story="FTX ch1: tribe fill + acid tease + thin air >6k so intro is not flat",
        ),
        "predrop": dict(
        ...
        "predrop_rise": dict(
            kick_mul=0.55, bass_mul=0.55, hats_mul=0.40, oh_mul=0.18, perc_mul=0.18,
            stretch_mul=0.42, acid_mul=0.45, vox_mul=0.85, click_mul=0.25, valve=0.82, air=0.55,
            hat_c="hh3c", hat_o="hh3o", rim="rim2", perc="junk", stretch="189_SynLP-1.wav",
            story="FTX ch2: acid accent + hats + filter open into drop_1",
        ),
        "drop": dict(
...
def adsr_level(t_ms: float, a_ms: float, d_ms: float, s_lvl: float, r_ms: float,
               gate_ms: float, note_on: bool) -> float:
    """Simple ADSR while gate is open; after gate → release from current level."""
    if t_ms < 0:
        return 0.0
    if note_on and t_ms <= gate_ms:
        if t_ms < a_ms:
            return t_ms / max(1e-6, a_ms)
        if t_ms < a_ms + d_ms:
            x = (t_ms - a_ms) / max(1e-6, d_ms)
            return 1.0 + (s_lvl - 1.0) * x
        return s_lvl
    # release
    # approximate: start from sustain (or attack peak if short gate)
    if note_on:
        # still gated path shouldn't land here
        return s_lvl
    # caller tracks release start level; this helper used only for gated phase
    return 0.0


def render_poly_acid(pattern, cycle: int, grit=None, roll=True,
                     macro_open: float = 1.0) -> list[float]:
    """Mental Tribe / 303 recipe (FineTuniX):
    - single saw
    - LPF 24 dB (cascaded 12 dB SVF x2), res ~70–90%
    - filter env A5/D300/S20/R200, amount +70%
    - amp env A2/D400/S60/R200
    - light sat; mid presence; HPF applied later in section
    Pattern index = global_16th % cycle (13/15 vs 4/4 kick).
    macro_open: 0..1+ multiplies filter open (arrangement automation).
    """
    grit = classic.GRIT if grit is None else grit
    step = (60.0 / BPM) / 4.0
    total_steps = classic.BARS * 16
    out = [0.0] * classic.N
    # dual 12 dB stages → ~24 dB
    l1 = b1 = l2 = b2 = 0.0
    phase = 0.0
    ...
    a_rel0 = 0.0
    # recipe timings (ms)
    FA, FD, FS, FR = 5.0, 300.0, 0.20, 200.0
    AA, AD, AS, AR = 2.0, 400.0, 0.60, 200.0
    # res 80% → feedback coeff (SVF q lower = more resonant)
    res_pct = 0.82
    res_q = max(0.04, 0.32 * (1.0 - 0.75 * res_pct))  # ~0.12 at 82%
    base_cut0 = classic.BASE_CUT0
    # amount +70% of env into cutoff span
    env_amt = classic.ENV_AMT * 0.70 / 0.70  # keep ENV_AMT as full +70% calibrated span
    grit_drive = grit
    step_idx = -1
    note_gate = False
    accent = False
    ...
    def tick_adsr(stage, t, rel0, a_ms, d_ms, s_lvl, r_ms, gated):
        if gated:
            ...
            if stage == "attack":
                ...
                if t >= a_ms:
                    stage, t = "decay", 0.0
                    lvl = 1.0
                return stage, t, lvl, rel0
            if stage == "decay":
                x = t / max(1e-6, d_ms)
                lvl = 1.0 + (s_lvl - 1.0) * min(1.0, x)
                if t >= d_ms:
                    stage, t = "sustain", 0.0
                    lvl = s_lvl
                return stage, t, lvl, rel0
            # sustain
            return "sustain", t + ms_per, s_lvl, rel0
        # gate off → release
        if stage == "idle":
        ...
        if stage != "release":
            # capture current
            if stage == "attack":
                rel0 = t / max(1e-6, a_ms)
            elif stage == "decay":
                x = t / max(1e-6, d_ms)
                rel0 = 1.0 + (s_lvl - 1.0) * min(1.0, x)
            else:
                rel0 = s_lvl
            stage, t = "release", 0.0
        t += ms_per
        lvl = rel0 * max(0.0, 1.0 - t / max(1e-6, r_ms))
        if t >= r_ms or lvl < 1e-4:
            return "idle", 0.0, 0.0, 0.0
        return stage, t, lvl, rel0
    ...
    for i in range(classic.N):
        ...
        if s != step_idx:
            ...
            if roll:
                ...
                roll_amt = 0.0
            # slow macro filter breathe every ~8 bars (LRA via automation)
            breathe = 0.78 + 0.22 * (0.5 + 0.5 * math.sin(2.0 * math.pi * (bar / 8.0)))
            render_poly_acid._base = (base_cut0 + roll_amt) * macro_open * breathe  # type: ignore
            render_poly_acid._q = res_q  # type: ignore
            p = pattern[st]
            if p["note"] > 0:
                target_hz = p["note"]
                if not p["slide"]:
                    cur_hz = target_hz
                note_gate = True
                accent = bool(p["accent"])
                # retrigger envelopes
                f_stage = a_stage = "attack"
                f_t = a_t = 0.0
            else:
                note_gate = False
        ...
        if accent:
            local_q = max(0.05, local_q * 0.85)  # more bite on accents

        cur_hz += (target_hz - cur_hz) * (0.012 if note_gate else 0.004)
        f_stage, f_t, f_env, f_rel0 = tick_adsr(f_stage, f_t, f_rel0, FA, FD, FS, FR, note_gate)
        a_stage, a_t, a_env, a_rel0 = tick_adsr(a_stage, a_t, a_rel0, AA, AD, AS, AR, note_gate)

        phase += cur_hz / SR
        phase -= math.floor(phase)
        # single saw
        osc = (2.0 * phase - 1.0) * a_env * (0.55 if accent else 0.42)

        # cutoff: base + filter-env * amount (+70% recipe)
        cut = base_cut + f_env * env_amt
        cut = dna.clamp(cut, 80.0, 12000.0)
        f = 2 * math.sin(math.pi * dna.clamp(cut / SR, 0.0001, 0.45))
        # stage 1
        l1 += f * b1
        h1 = osc - l1 - local_q * b1
        b1 += f * h1
        # stage 2 (24 dB)
        l2 += f * b2
        h2 = l1 - l2 - local_q * b2
        b2 += f * h2
        y = dna.tanh_drive(l2, grit_drive)
        out[i] = y
    return out
...
def render_section(idx: int, name: str, bars: int, mode: str, shots, phrases, packs, out_prefix: Path):
    ...
    for bar in range(bars):
        ...
        elif mode == "intro_tribe":
            for qi, step in enumerate((0, 8)):
                place(trk_k, kick_thin, base + step * 0.25, 0.75)
            if bar % 2 == 1:
                place(trk_k, kick_thin, base + 2.0, 0.45)
                place(trk_k, click, base + 2.0, 0.2)
            if bar % 4 == 3:
                place(trk_k, kick, base + 3.0, 0.55)
        elif mode == "predrop":
            if bar % 2 == 0:
        ...
        elif mode == "predrop_rise":
            dens = bar / max(1, bars - 1)
            steps = (0, 8) if dens < 0.5 else (0, 4, 8, 12)
            for qi, step in enumerate(steps):
                place(trk_k, kick_thin if dens < 0.7 else kick, base + step * 0.25, 0.65 + 0.25 * dens)
                if dens > 0.55:
                    place(trk_k, click, base + step * 0.25, 0.18 * dens)
        ...
        if mode in ("drop", "drop_climax") and mid_ok:
            ...
            for st in (6, 14):
                group6.trig("A", hat_c, base + st * 0.25 + swing_offset(st), 0.32)
            if bar % 2 == 1:
                group6.trig("B", hat_o, base + 14 * 0.25 + swing_offset(14), 0.18)
            if bar % 4 == 2:
                group7.trig("A", hat7_c, base + 10 * 0.25 + swing_offset(10), 0.2)
        elif mode == "predrop" and bar > 2:
            group6.trig("A", hat_c, base + 14 * 0.25 + swing_offset(14), 0.22)
        elif mode == "predrop_rise":
            dens = bar / max(1, bars - 1)
            steps = [14] if dens < 0.35 else ([6, 14] if dens < 0.7 else [2, 6, 10, 14])
            for st in steps:
                group6.trig("A", hat_c, base + st * 0.25 + swing_offset(st), 0.28 + 0.3 * dens)
            if dens > 0.5:
                group6.trig("B", hat_o, base + 14 * 0.25, 0.18 + 0.15 * dens)
        elif mode == "outro" and bar < bars * 0.5:
            group6.trig("A", hat_c, base + 14 * 0.25, 0.18)

        # PERC
        if mid_ok and rim is not None and mode in ("drop", "drop_climax", "build", "groove", "drop3", "drop3_outro", "intro_tribe", "predrop_rise"):
            place(trk_p, rim, base + 1.0, 0.32)
            if mode in ("drop_climax", "drop3") and bar % 2 == 1:
                place(trk_p, rim, base + 2.75, 0.22)
        if perc is not None and mode in ("drop_climax", "groove", "drop3", "break2", "drop3_outro", "intro_tribe", "predrop_rise"):
            if bar % 4 == (2 if mode != "break2" else 1):
                place(trk_p, perc, base + 1.75 + swing_offset(7), 0.14)

        # STRETCH
        if stretch_one is not None:
            if mode in ("intro", "predrop", "break", "break2", "intro_tribe"):
                if bar % 2 == 0:
                    place(trk_st, stretch_one, base, 0.5 if mode != "intro_tribe" else 0.42)
            elif mode == "predrop_rise":
        ...
        if phrases:
            if mode == "predrop":
                ...
                pack = packs[bar % len(packs)] if packs else phrases[bar % len(phrases)]
                place(trk_v, pack, base + 0.5, 0.7)
                if bar % 2 == 0:
                    place(trk_v, phrases[bar % len(phrases)], base + 2.5, 0.5)
            elif mode == "intro_tribe" and bar % 4 == 1:
                place(trk_v, phrases[bar % len(phrases)], base + 1.5, 0.4)
    ...
    acid_drive = {
        "intro": 0.06, "intro_tribe": 0.32, "predrop": 0.14, "predrop_rise": 0.48,
        "break": 0.18, "break2": 0.15, "build": 0.5,
        "drop": 0.42, "drop_climax": 0.62, "groove": 0.2, "drop3": 0.5,
        "outro": 0.08, "drop3_outro": 0.38,
    }.get(mode, 0.2)
    # Filter macro open: quieter chapters more closed → LRA via automation
    macro_open = {
        "intro": 0.55, "intro_tribe": 0.85, "predrop": 0.65, "predrop_rise": 1.05,
        "break": 0.5, "break2": 0.45, "build": 0.95,
        "drop": 1.0, "drop_climax": 1.12, "groove": 0.7, "drop3": 1.0,
        "outro": 0.4, "drop3_outro": 0.85,
    }.get(mode, 0.9)
    cycle = 15 if mode in ("intro", "intro_tribe", "predrop", "predrop_rise", "drop", "build", "drop_climax", "outro") else 13
    if mode in ("drop3", "drop3_outro", "groove", "break2"):
        cycle = 13
    if mode in ("break",):
        cycle = 15
    poly_pat = poly_last_step_pattern(dna.SEED + 303 + idx * 17 + cycle, cycle=cycle)
    acid_full = render_poly_acid(poly_pat, cycle=cycle, grit=classic.GRIT, roll=True, macro_open=macro_open)
    acid = [x * acid_drive for x in acid_full[:N]]
    if len(acid) < N:
        acid.extend([0.0] * (N - len(acid)))
    if acid_drive > 0:
        # FTX: HPF ~160 Hz keeps kick clear; presence 2–4 kHz; light LPF (keep mids)
        acid = one_pole(acid, ACID_HPF_HZ, mode="hpf")
        acid = peaking_eq(acid, ACID_PRESENCE_HZ, ACID_PRESENCE_DB, q=1.1)
        acid = peaking_eq(acid, 1800.0, 2.0, q=0.9)
        if mode in ("drop", "drop_climax", "drop3", "drop3_outro"):
            acid = one_pole(acid, 4200.0, mode="lpf")  # was 1350 — mid wall fixed via presence, not dark cut
            ...
            acid = one_pole(acid, 5200.0, mode="lpf")

    # Ring Mod lead — invert cycle vs 303 so the two voices weave (poly)
    rm_drive = {
        "intro": 0.04, "intro_tribe": 0.18, "predrop": 0.10, "predrop_rise": 0.28,
        "break": 0.12, "break2": 0.08, "build": 0.36,
    ...
    # Mono bed: kick + bass stay perfectly centered
    mono_bed = [0.0] * N
    for i in range(N):
        mono_bed[i] = trk_k[i] * g_kick + trk_b[i] * g_bass

    # Wide-eligible: acid + ringmod (+ thin air >6 kHz)
    wide_mono = [0.0] * N
    air_n = thin_air_noise(N, seed=dna.SEED + idx * 19 + hash(mode) % 997)
    air_amt = AIR_NOISE_G * kit["air"]
    for i in range(N):
        wide_mono[i] = acid[i] * g_acid + rm[i] * g_rm + air_n[i] * air_amt
    acid_L, acid_R = haas_widen(wide_mono, width=ACID_WIDTH * (0.6 + 0.4 * kit["air"]))

    # Other mono-ish layers (hats/perc/stretch/vox) — slight send width later
    other = [0.0] * N
    for i in range(N):
        other[i] = (
    ...
    for i in range(N):
        send_src[i] = (
            ...
            + trk_st[i] * g_st * 0.2 + air_n[i] * air_amt * 0.4
        )
    Lpp, Rpp = pingpong_stereo(send_src, wet=DELAY_WET * (0.5 + 0.5 * air), feedback=DELAY_FB)
    left = [0.0] * N
    right = [0.0] * N
    for i in range(N):
        left[i] = mono_bed[i] + other[i] + acid_L[i] + (Lpp[i] - send_src[i]) * 0.85
        right[i] = mono_bed[i] + other[i] + acid_R[i] + (Rpp[i] - send_src[i]) * 0.85
...
def measure_lufs(path: Path) -> dict:
    """ffmpeg ebur128 integrated / true-peak / LRA — prefer Summary block."""
    ...
    try:
        ...
        out["raw"] = txt[-3000:]
        # Prefer trailing Summary section (not running -70 start line)
        summ = txt
        if "Summary:" in txt:
            summ = txt.rsplit("Summary:", 1)[-1]
        m_i = re.search(r"I:\s+(-?[0-9.]+)\s+LUFS", summ)
        m_lra = re.search(r"LRA:\s+(-?[0-9.]+)\s+LU", summ)
        m_tp = re.search(r"Peak:\s+(-?[0-9.]+)\s+dBFS", summ)
        if m_tp is None:
            m_tp = re.search(r"True peak:\s+(-?[0-9.]+)\s+dBTP", summ, re.I)
        if m_tp is None:
            m_tp = re.search(r"Peak:\s+(-?[0-9.]+)\s+dBTP", summ, re.I)
        if m_i:
    ...
    for sid, sname, mode, bars in ABLETON_SCENES:
        ...
        for tidx, key, cname in pairs:
            ...
            if ok_clip:
                ...
                print("DEFER clip (use load_mentalclean_morph_continue_ftx.py):", cname)
...
def main():
    errors = []
    print("=== MentalClean Molecular MORPH @", BPM, "FTX (FineTuniX polish) ===")
    ...
    print("=== Bounce MP3 (FTX limiter -1.5 dBTP + loudnorm) ===")
    try:
    ...
    # 2) Ableton 8-scene shorter clips (skip render when --skip-ableton)
    skip_ab = "--skip-ableton" in sys.argv
    ab_paths = {}
    ab_meta = []
    if skip_ab:
        print("=== Ableton clips SKIP (--skip-ableton) ===")
    ...
    # LUFS of bounced FTX (do not overwrite v1/v2)
    ...
    # notes
    v2_ref = {"I": -9.9, "TP": -0.7, "LRA": 8.9}
    lines = [
        "# EvAIx MentalClean Molecular MORPH @162 FineTuniX (FTX)",
        f"MP3: `{MP3}`",
        "Also deliver to /workspace/ and /workspace/exports/ (same filename).",
        f"Duration: {dur:.2f}s · BPM {BPM}",
        "NEW bounce — does NOT overwrite v1 or v2.",
        ...
        "",
        "## Limiter / master (FineTuniX #1)",
        "- loudnorm I=-7.5 TP=-1.5 LRA=7, then volume=-0.5dB + alimiter limit=0.841 (≈−1.5 dBTP, level=disabled)",
        "- Final encode-safe ceiling: no TP > −1.0 on MP3 (target −1.5 headroom before lame)",
        "- Soft glue compressor ratio 1.30 (NOT harder master squash for LRA)",
        "- Mid presence EQ: +1.4@900 / +1.6@1800 / +2.0@2800 / +2.0@6500",
        f"- Measured: I={lufs.get('I')} LUFS  TP={lufs.get('TP')} dB  LRA={lufs.get('LRA')} LU",
        f"- vs v2: I={v2_ref['I']} TP={v2_ref['TP']} LRA={v2_ref['LRA']}",
        "",
        "## 303 / Mental Tribe recipe applied (FineTuniX #2)",
        "- Osc: single saw",
        "- LPF 24 dB (cascaded SVF×2), res ≈82%",
        "- Filter env A5 / D300 / S20 / R200 ms; amount +70% into cutoff",
        "- Amp env A2 / D400 / S60 / R200 ms",
        "- Light sat (GRIT 1.55); presence boost ~1.8–2.8 kHz; HPF acid 160 Hz",
        "- Thin air noise layer >6 kHz (AIR_NOISE_G)",
        f"- ACID_G={ACID_G}; macro_open per chapter for filter automation",
        "",
        "## Early chapters / macrodynamics (FineTuniX #3)",
        "- Split first ~71s into 4 chapters: intro → intro_tribe → predrop → predrop_rise",
        "- intro_tribe: tribe fill + acid tease + air (was flat plateau)",
        "- predrop_rise: acid accent + hats + filter open into drop_1",
        "- LRA via filter/layer automation + quieter breaks — not more master squash",
        "",
        "## Stereo (FineTuniX #4)",
        f"- Kick/bass mono bed; Haas width on acid/FX only (ACID_WIDTH={ACID_WIDTH}, {ACID_HAAS_MS} ms)",
        "",
        "## Ring Mod (kept from v2)",
        f"- OSC1/OSC2: `{RM_WAVE1}` + `{RM_WAVE2}` · ModDepth={RM_MOD_DEPTH} · Detune={RM_DETUNE_CENTS}c",
        f"- Mod→Pitch {RM_MOD_TO_PITCH} @ {RM_MOD_SPEED_HZ} Hz ±{RM_MOD_PITCH_DEPTH}; HPF {RM_HPF_HZ}/LPF {RM_LPF_HZ}",
        "",
        "## Hat choke (kept)",
        f"- EMX 6A/6B + 7A/7B exclusive; fade {CHOKE_FADE_MS} ms; B wins if simultaneous",
        "",
    ...
    if ab_meta:
        ...
        lines.append("- (skipped with --skip-ableton)")
    ...
    print("V2_REF_I", v2_ref["I"], "V2_REF_TP", v2_ref["TP"], "V2_REF_LRA", v2_ref["LRA"])
    ...
    print("ERRORS", errors)
    print("DONE build_mentalclean_molecular_morph_162_ftx")
