# -*- coding: utf-8 -*-
"""EvAIx Molecular STORY @168 — Ableton session chapters + full MP3 bounce.

AbletonMCP: 8 named-via-clips story scenes with DISTINCT stems per row.
Full Mix Learner 10-section timeline bounced to MP3 (CRISP acoustic).
"""
...
OUT = BRIDGE / "samples" / "crisp-dna" / "esx1-molecular-story-168"
...
MP3 = Path(r"C:\Users\Gebruiker\Downloads\EvAIx_MolecularSTORY_Ableton_168.mp3")
NOTES = OUT / "molecular_story_ableton_168_notes.md"
...
def render_section(idx: int, name: str, bars: int, mode: str, shots, phrases, packs, out_prefix: Path):
    ...
    for bar in range(bars):
        ...
        if mode in ("drop", "drop_climax") and mid_ok:
            ...
            if bar % 2 == 1:
                place(trk_oh, hat_o, base + 14 * 0.25 + swing_offset(14), 0.3)
            if mode == "drop_climax" and bar % 4 == 0:
                for st in (3, 7, 11):
                    place(trk_h, hat_c, base + st * 0.25 + swing_offset(st), 0.2)
        elif mode == "drop3" and mid_ok:
            # mute games every 4 bars
            if (bar // 4) % 2 == 0:
                for st in (2, 6, 10, 14):
                    place(trk_h, hat_c, base + st * 0.25 + swing_offset(st), 0.45)
                place(trk_oh, hat_o, base + 6 * 0.25 + swing_offset(6), 0.28)
            else:
                for st in (6, 14):
            ...
            if bar < bars // 2:
                ...
                if bar % 2 == 0:
                    place(trk_h, hat_c, base + 14 * 0.25, 0.25)
        elif mode == "build":
            dens = bar / max(1, bars - 1)
            steps = [14]
            if dens > 0.3:
            ...
            for st in steps:
                place(trk_h, hat_c, base + st * 0.25 + swing_offset(st), 0.25 + 0.35 * dens)
            if dens > 0.6:
                place(trk_oh, hat_o, base + 14 * 0.25, 0.2 + 0.2 * dens)
        elif mode == "groove":
            for st in (6, 14):
                place(trk_h, hat_c, base + st * 0.25 + swing_offset(st), 0.38)
            if bar % 2 == 1:
                place(trk_h, hat_c, base + 3 * 0.25 + swing_offset(3), 0.22)
        elif mode == "predrop" and bar > 4:
            place(trk_h, hat_c, base + 14 * 0.25 + swing_offset(14), 0.22)
        elif mode == "outro" and bar < bars * 0.5:
            place(trk_h, hat_c, base + 14 * 0.25, 0.18)
    ...
    # Acid — unique phrase seed per section
    acid_drive = {
    ...
    for i in range(N):
        send_src[i] = (
            acid[i] * g_acid * 0.3 * air + trk_h[i] * g_hats * 0.5 * air
    ...
    meta = {
        ...
        "gains": dict(kick=g_kick, bass=g_bass, hats=g_hats, vox=g_vox, acid=g_acid),
    }
    print(f"section {idx} {name} mode={mode} bars={bars} vox_g={g_vox:.2f} | {kit['story']}")
    return paths, meta
...
def ableton_load(scene_paths, scene_meta):
    ...
    ok(send("set_tempo", {"tempo": BPM}), "tempo 168")
    ...
    idxs = [i for i in (kick_idx, hats_idx, oh_idx, perc_idx, acid_idx, stretch_idx, vox_idx, jam_idx) if i is not None]
    ...
    for sid, sname, mode, bars in ABLETON_SCENES:
        ...
        for tidx, key, cname in pairs:
            ...
            if key != "mix" and not morph.layer_has_signal(p):
                continue
            # shorter clips already 8 bars — still retry shorter if hang by timeout fail
            ok_clip = load_audio(tidx, sid, p, cname)
            if not ok_clip:
                # retry once
                ok_clip = load_audio(tidx, sid, p, cname)
            if ok_clip:
...
def main():
    errors = []
    print("=== Molecular STORY Ableton @", BPM, "===")
    ...
    # notes
    lines = [
        "# EvAIx Molecular STORY Ableton @168",
        f"MP3: `{MP3}`",
    ...
    for m in full_meta:
        lines.append(
            ...
            f"[hat={m['hat']} rim={m['rim']} perc={m['perc']} vox_g={m['gains']['vox']:.2f}]"
        )
    ...
    print("ERRORS", errors)
    print("DONE build_molecular_story_ableton_168")
