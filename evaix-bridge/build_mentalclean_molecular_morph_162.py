# -*- coding: utf-8 -*-
"""EvAIx MentalClean + Molecular MORPH @162 — clean kick/bass DNA + sectional morphs.
...
OUT = BRIDGE / "samples" / "crisp-dna" / "mentalclean-molecular-morph-162"
...
MP3 = Path(r"C:\Users\Gebruiker\Downloads\EvAIx_MentalClean_MolecularMorph_162.mp3")
NOTES = OUT / "mentalclean_molecular_morph_162_notes.md"
...
def render_section(idx: int, name: str, bars: int, mode: str, shots, phrases, packs, out_prefix: Path):
    ...
    print(f"section {idx} {name} mode={mode} bars={bars} poly={cycle} vox_g={g_vox:.2f} | {kit['story']}")
    return paths, meta
...
def ableton_load(scene_paths, scene_meta):
    ...
    for sid, sname, mode, bars in ABLETON_SCENES:
        ...
        for tidx, key, cname in pairs:
            ...
            ok_clip = load_audio(tidx, sid, p, cname, timeout=55.0)
            if not ok_clip:
                # one short retry then leave for continue script
                ok_clip = load_audio(tidx, sid, p, cname, timeout=40.0)
            if ok_clip:
                loaded.append(cname)
            else:
                print("DEFER clip (use load_mentalclean_morph_continue.py):", cname)
        # locator for chapter
...
def main():
    errors = []
    print("=== MentalClean Molecular MORPH @", BPM, "===")
    ...
    # notes
    lines = [
        "# EvAIx MentalClean Molecular MORPH @162",
        f"MP3: `{MP3}`",
    ...
    print("ERRORS", errors)
    print("DONE build_mentalclean_molecular_morph_162")
