# -*- coding: utf-8 -*-
"""ESX-1 Valve Force SwingFX Live @ 162 — tutorial features dry-run #2.

DNA (extends Valve Force Live):
- Valve Force master: asymmetric tanh Tube Gain ~2.1
- One-shots via CRISP esx-1 convert (HP30 + alimiter 0.94 mono 44.1)
- SWING ~59% on closed hats / perc odd 16ths ONLY (kick stays straight)
- FILTER MOTION: tempo-synced LFO Autofilter + Break_Filter open-close sweeps
- LFO MOD: Speed/Depth pitch wobble light on acid + stretch
- FX SEND bus (baked): Decimator JunkPerc; short/ping-pong Delay hats+acid; Valve master
- Stretch: SynLP/PercLP rubberband; 8 scenes x 24 bars @162
"""
...
OUT = BRIDGE / "samples" / "crisp-dna" / "esx1-valveforce-swingfx"
...
MP3 = Path(r"C:\Users\Gebruiker\Downloads\EvAIx_ESX1_ValveForce_SwingFX_162.mp3")
...
ENV_DECAY = 0.9970
OSC_KIND = "saw"
TUBE_GAIN = 2.12  # Valve Force Tube Gain feel (~2.0–2.2)

# --- Tutorial Swing / LFO / FX Send knobs (Olean ESX ~LCD Swing) ---
SWING_PCT = 0.59  # ~55–62 window; LCD-ish 59%
# Map 50%→0 delay, 66.7%→triplet (1/3 of 16th toward next 8th)
SWING_DELAY = (SWING_PCT - 0.5) / ((2.0 / 3.0) - 0.5) * (0.25 / 3.0)
# FX send (Edit1=time Edit2=level style)
DELAY_TIME_BEATS = 0.375  # dotted-8th-ish short / ping-pong tap
DELAY_WET = 0.16  # Edit2 level — keep low
DELAY_FB = 0.26
DECIM_BITS = 10
DECIM_RATE_DIV = 3
LFO_PERIOD_BEATS = 8.0  # 2 bars @ 4/4
LFO_CUT_DEPTH = 0.62
LFO_PITCH_DEPTH = 0.0038  # light Speed/Depth pitch
...
SWING_METHOD = f"odd-16th delay {SWING_DELAY:.4f} beats (~{int(SWING_PCT*100)}% swing); kick straight"
FX_SEND_METHOD = (
    f"Decimator bits={DECIM_BITS} rate_div={DECIM_RATE_DIV} JunkPerc only; "
    f"ping-pong Delay time={DELAY_TIME_BEATS}b wet={DELAY_WET} fb={DELAY_FB} on hats/acid; "
    f"LFO cut period={LFO_PERIOD_BEATS}b depth={LFO_CUT_DEPTH}; pitch depth={LFO_PITCH_DEPTH}"
)
...
def apply_motion_filter(samples: list[float], sid: int) -> list[float]:
    """Stepped cutoff/res every 2-4 bars — ESX motion seq feel (kept as base)."""
    out = [0.0] * len(samples)
...
def swing_offset(st: int) -> float:
    """Offset closed-hat / perc odd 16ths late. Never use on kick."""
    return SWING_DELAY if (st % 2 == 1) else 0.0


def apply_lfo_autofilter(
    samples: list[float],
    sid: int,
    base_cut: float = 900.0,
    depth: float = LFO_CUT_DEPTH,
    period_beats: float = LFO_PERIOD_BEATS,
    res: float = 0.22,
    mode: str = "lpf",
) -> list[float]:
    """Tempo-synced Autofilter-like LPF/HPF (Oct Bass / Break_Filter sweeps)."""
    out = [0.0] * len(samples)
    l = b = 0.0
    bps = BPM / 60.0
    for i, x in enumerate(samples):
        t_beats = (i / SR) * bps
        if sid == 5:
            # Break_Filter: 4-bar open-close triangle sweep
            phase = (t_beats / 16.0) % 1.0
            tri = 1.0 - abs(2.0 * phase - 1.0)
            cut = 260.0 + tri * 3400.0
            q = 0.30 + tri * 0.22
            wet = 0.86
        elif sid == 6:
            phase = math.sin(2.0 * math.pi * t_beats / max(1e-6, period_beats * 0.75))
            cut = base_cut * (1.15 + depth * 0.95 * phase)
            q = res + 0.10 * abs(phase)
            wet = 0.78
        elif sid >= 3:
            phase = math.sin(2.0 * math.pi * t_beats / max(1e-6, period_beats))
            cut = base_cut * (1.0 + depth * 0.58 * phase)
            q = res + 0.05 * abs(phase)
            wet = 0.68
        else:
            phase = math.sin(2.0 * math.pi * t_beats / max(1e-6, period_beats * 1.25))
            cut = base_cut * (0.92 + depth * 0.35 * phase)
            q = res * 0.85
            wet = 0.55
        cut = dna.clamp(cut, 80.0, 8500.0)
        f = 2.0 * math.sin(math.pi * dna.clamp(cut / SR, 0.0001, 0.45))
        l += f * b
        h = x - l - q * b
        b += f * h
        y = h if mode == "hpf" else l
        # Peak / Break also blend a touch of HPF motion on the dry
        if sid == 5 and (i // max(1, int(0.5 * SR))) % 2 == 1:
            y = 0.65 * l + 0.35 * h
...
def apply_pitch_lfo(
    samples: list[float],
    depth: float = LFO_PITCH_DEPTH,
    period_beats: float = LFO_PERIOD_BEATS,
) -> list[float]:
    """Light Speed/Depth pitch wobble via variable-rate read."""
    if not samples:
        return samples
    out = [0.0] * len(samples)
    bps = BPM / 60.0
    ...
    for i in range(len(samples)):
        t_beats = (i / SR) * bps
        rate = 1.0 + depth * math.sin(2.0 * math.pi * t_beats / max(1e-6, period_beats))
        pos += rate
        idx = int(pos)
        frac = pos - idx
        if idx >= last:
...
def short_delay_mono(
    samples: list[float],
    time_beats: float = DELAY_TIME_BEATS,
    feedback: float = DELAY_FB,
    wet: float = DELAY_WET,
) -> list[float]:
    """Short delay send (Edit1=time Edit2=level) — mono stem color."""
    d = max(1, int(time_beats * 60.0 / BPM * SR))
    out = [0.0] * len(samples)
    delay_line = [0.0] * len(samples)
    for i, x in enumerate(samples):
        delayed = delay_line[i - d] if i >= d else 0.0
        y = x + delayed * wet
        # write feedback into delay line from wet path
        delay_line[i] = x + delayed * feedback
        out[i] = y
    return out
...
def pingpong_stereo(
    ...
    """Ping-pong delay for section mix (L/R bounce)."""
    d = max(1, int(time_beats * 60.0 / BPM * SR))
    L = list(samples)
    R = list(samples)
    for tap in range(4):
...
def ensure_oneshots():
    ...
    if junk:
        junk = bitcrush_mild([dna.tanh_drive(x, 1.35) for x in junk], bits=DECIM_BITS, rate_div=DECIM_RATE_DIV)
...
def render_scene(sid, name, shots, pattern):
    ...
    for bar in range(BARS):
        ...
        # --- HATS (SWING on odd 16ths; kick never swung) ---
        if sid == 0:
            pass
        elif sid == 1:
            # 8ths → expand to swung 16ths feel on offbeats
            for st in range(16):
                if st % 2 == 1:
                    ...
                    place(trk_h, hat_c, base + st * 0.25, 0.55)
        elif sid == 2:
            for st in range(16):
                g = 0.74 if st % 2 == 0 else 0.46
                place(trk_h, hat_c, base + st * 0.25 + swing_offset(st), g)
            for st in (6, 14):
                place(trk_oh, hat_o, base + st * 0.25 + swing_offset(st), 0.34)
        elif sid in (3, 4):
            for st in range(16):
                place(trk_h, hat_c, base + st * 0.25 + swing_offset(st), 0.76 if st % 2 == 0 else 0.46)
            for st in (2, 6, 10, 14):
                place(trk_oh, hat_o, base + st * 0.25 + swing_offset(st), 0.4 if sid == 4 else 0.34)
        elif sid == 5:
            for st in range(0, 16, 2):
                place(trk_h, hat_c, base + st * 0.25 + swing_offset(st), 0.36)
            for st in (6, 14):
                place(trk_oh, hat_o, base + st * 0.25 + swing_offset(st), 0.28)
        elif sid == 6:
            for st in range(16):
                place(trk_h, hat_c, base + st * 0.25 + swing_offset(st), 0.82 if st % 2 == 0 else 0.52)
            for st in (2, 6, 10, 14):
                place(trk_oh, hat_o, base + st * 0.25 + swing_offset(st), 0.46)
        elif sid == 7 and bar < 8:
            for st in range(0, 16, 2):
                place(trk_h, hat_c, base + st * 0.25 + swing_offset(st), 0.42 if st % 4 == 0 else 0.24)

        # --- ONE-SHOT perc: rim/snare/junk (swung offbeats) ---
        if sid in (3, 4, 6):
            if rim is not None:
                # beat 1.0 = step 4 (even) straight; add swung ghost on odd 16th
                place(trk_p, rim, base + 1.0, 0.55 if sid != 6 else 0.65)
                place(trk_p, rim, base + 3.0, 0.5)
                place(trk_p, rim, base + 1.25 + swing_offset(5), 0.28)
            if snare is not None and sid >= 4:
                place(trk_p, snare, base + 1.0, 0.35)
            if junk is not None:
                if bar % 2 == 0:
                    # 0.75 = step 3 (odd) → swing late
                    place(trk_p, junk, base + 0.75 + swing_offset(3), 0.42 if sid < 6 else 0.5)
                if sid == 6 and bar % 2 == 1:
                    place(trk_p, junk, base + 2.75 + swing_offset(11), 0.38)
            if synp is not None and sid == 6 and bar % 4 == 0:
                place(trk_p, synp, base + 1.75 + swing_offset(7), 0.32)
        elif sid == 5 and junk is not None and bar % 2 == 0:
            place(trk_p, junk, base + 2.75 + swing_offset(11), 0.28)
    ...
    # FILTER MOTION: stepped motion seq + tempo-synced LFO Autofilter
    if any(abs(x) > 1e-6 for x in trk_st):
        trk_st = apply_motion_filter(trk_st, sid)
        trk_st = apply_lfo_autofilter(trk_st, sid, base_cut=1100.0, depth=LFO_CUT_DEPTH, mode="lpf")
        if sid >= 3:
            trk_st = apply_pitch_lfo(trk_st, depth=LFO_PITCH_DEPTH * (1.2 if sid == 5 else 1.0))
    if acid_drive > 0:
        if sid >= 3:
            acid = apply_motion_filter(acid, sid)
        acid = apply_lfo_autofilter(
            acid,
            sid,
            base_cut=700.0 if sid != 5 else 550.0,
            depth=LFO_CUT_DEPTH * (1.15 if sid >= 5 else 0.9),
            mode="lpf",
        )
        if sid >= 2:
            acid = apply_pitch_lfo(acid, depth=LFO_PITCH_DEPTH)

    # FX SEND: short delay on hats + acid (wet low); junk already decimated at oneshot
    if any(abs(x) > 1e-6 for x in trk_h):
        trk_h = short_delay_mono(trk_h, wet=DELAY_WET * 0.85)
    if any(abs(x) > 1e-6 for x in trk_oh):
        trk_oh = short_delay_mono(trk_oh, wet=DELAY_WET * 0.7)
    if acid_drive > 0:
        acid = short_delay_mono(acid, wet=DELAY_WET * 0.9)
    ...
    # Ping-pong delay send on hats+acid into stereo mix
    send_src = [0.0] * N
    for i in range(N):
        send_src[i] = acid[i] * acid_g * 0.55 + trk_h[i] * hats_g * 0.7 + trk_oh[i] * oh_g * 0.45
    ...
    print(
        "scene", sid, name,
        "swing", round(SWING_DELAY, 4),
        "stretch_peak", round(max(abs(x) for x in trk_st), 4),
        "acid_peak", round(max(abs(x) for x in acid), 4),
    )
    return paths
...
def main():
    errors = []
    print("=== ESX-1 Valve Force SwingFX @", BPM, "bars/scene", BARS, f"sec/scene={N/SR:.2f} ===")
    print("VALVE", VALVE_METHOD)
    print("SWING", SWING_METHOD)
    print("FX_SEND", FX_SEND_METHOD)
    ...
    if stretch_idx is not None and stretch_idx not in (kick_idx, hats_idx, perc_idx, oh_idx, acid_audio_idx):
        ...
        print("unmuted stretch", stretch_idx)
    ...
    print("ONESHOTS", shots["used"])
    print("TRACK_MAP", "E-Kick=SinKick | E-Hats=HH-1C swung | E-OHat=HH-1O | E-Perc=Junk/Rim/SD decimated | ESX Stretch=SynLP/PercLP LFO | Acid 303=saw+LFO+delay")
    ...
    print("TEMPO", tempo)
    print("ERRORS", errors)
    print("DONE build_esx1_valveforce_swingfx")
