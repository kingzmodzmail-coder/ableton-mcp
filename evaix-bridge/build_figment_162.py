...
MP3_NAME = "EvAIx_Figment_162.mp3"
...
NOTES_PATH = (
    Path("/workspace/exports/EvAIx_Figment_162_NOTES.md")
    if Path("/workspace/exports").exists()
    else OUT / "EvAIx_Figment_162_NOTES.md"
)
BRIDGE_COPY = Path(r"C:\Users\Gebruiker\ableton-mcp\evaix-bridge\build_figment_162.py")
...
ACID_G, MG_G, ATM_G, PAD_G, FX_G = 0.26, 0.18, 0.16, 0.22, 0.09
TUBE_GAIN = 1.55
...
def acid_303_note(freq: float, dur_s: float, accent: bool = False,
    ...
    """Acid LPF 303-style saw — cutoff 30–50% muted → open on drop."""
    n = max(8, int(dur_s * SR))
    t = np.arange(n) / SR
    # slide-ish slight pitch dive on accents
    pitch = freq * (1.0 + (0.06 if accent else 0.0) * np.exp(-t / 0.04))
    phase = np.cumsum(2 * np.pi * pitch / SR)
    saw = 2.0 * (phase / (2 * np.pi) - np.floor(0.5 + phase / (2 * np.pi)))
    # EG on cutoff
    cut0 = 280.0 + 900.0 * cutoff_open
    cut1 = cut0 + (1800.0 + 3200.0 * cutoff_open) * (1.15 if accent else 0.85)
    cut = cut0 + (cut1 - cut0) * np.exp(-t / (0.12 if accent else 0.20))
    cut = cut * (1.0 + 0.15 * res_boost)
    y_lo = one_pole(saw, float(np.percentile(cut, 20)), mode="lpf")
    y_hi = one_pole(saw, float(np.percentile(cut, 80)), mode="lpf")
    ...
    # EQ before sat; tame 3–6 kHz after grit
    y = one_pole(y * env * (1.15 if accent else 0.9), 180.0, mode="hpf")
    y = tanh_drive(y, 1.25 + 0.35 * cutoff_open)
    y = one_pole(y, 4800.0 - 800.0 * (1.0 - cutoff_open), mode="lpf")
    return y


def mg_lpf_note(freq: float, dur_s: float, accent: bool = False,
                cutoff_open: float = 0.30) -> np.ndarray:
    """MG LPF −8va sub-acid layer (VdW/fragile dual)."""
    n = max(8, int(dur_s * SR))
    t = np.arange(n) / SR
    phase = np.cumsum(2 * np.pi * freq / SR)
    sq = np.sign(np.sin(phase))
    cut = 180.0 + 1400.0 * cutoff_open + (900.0 if accent else 400.0) * np.exp(-t / 0.25)
    ...
    y = tanh_drive(y * env * 0.85, 1.15)
    y = one_pole(y, 90.0, mode="hpf")
    y = one_pole(y, 3200.0, mode="lpf")  # quiet tops
    return y
...
def kit_for_mode(mode: str, slot: str, shots: dict) -> dict:
    ...
    gains = {
        ...
        "drop_muted": dict(kick=1.0, bass=0.85, hats=0.45, oh=0.20, ride=0.70, perc=0.35,
                           acid=0.35, mg=0.40, atm=0.12, pad=0.35, fx=0.22, acid_open=0.22),
        "drop_open":  dict(kick=1.05, bass=1.0, hats=0.55, oh=0.30, ride=0.80, perc=0.45,
                           acid=1.0, mg=0.90, atm=0.08, pad=0.25, fx=0.28, acid_open=0.78),
        "drop_close": dict(kick=0.95, bass=0.70, hats=0.30, oh=0.08, ride=0.0, perc=0.20,
                           acid=0.45, mg=0.35, atm=0.20, pad=0.40, fx=0.25, acid_open=0.30),
...
def render_section(idx: int, name: str, bars: int, mode: str, slot: str, shots: dict):
    ...
    if dens > 0.01 and (g["acid"] > 0.05 or g["mg"] > 0.05):
        for step, note_i, accent, dur_steps in acid_poly13(bars, dens, seed):
            ...
            note = acid_303_note(freq, dur_steps * STEP_S, accent=accent, cutoff_open=acid_open,
                                 res_boost=0.3 if mode == "drop_open" else 0.0)
            place(trk_acid, note, step / 4.0, 1.1 if accent else 0.85)
            # MG −8va
            mg = mg_lpf_note(float(FS_LOW[note_i % len(FS_LOW)]), dur_steps * STEP_S * 1.1,
                             accent=accent, cutoff_open=acid_open * 0.85)
            place(trk_mg, mg, step / 4.0, 0.9)
    ...
    # macro LPF arcs per mode (Sync/acid LPF until drop opens)
    mono_bed = trk_k + trk_b + trk_h + trk_oh + trk_ride + trk_p + trk_atm + trk_pad
    wide_src = trk_acid + trk_mg + trk_fx
    ...
    if g["pad"] > 0.3:
        ...
        wl = wl + pl; wr = wr + pr
        mono_bed = mono_bed - trk_pad * 0.15  # avoid double
...
def bounce_mp3(mixes, mp3_path: Path) -> float:
    ...
    af = (
        ...
        "equalizer=f=4500:t=q:w=1.2:g=-1.5,"
        "equalizer=f=6500:t=q:w=1.2:g=-1.8,"
        "equalizer=f=10000:t=q:w=1.1:g=-2.5,"
...
def write_notes(dur: float, metas: list, lufs: dict, shots: dict, errors: list):
    t0 = 0.0
    lines = [
        "# EvAIx Figment — Vestiges slots 01–04 @ 162 F#",
        "",
        f"**MP3:** `{MP3_NAME}`",
        f"- Downloads: `C:\\\\Users\\\\Gebruiker\\\\Downloads\\\\{MP3_NAME}`",
        f"- Workspace: `/workspace/{MP3_NAME}`",
        f"- Exports: `/workspace/exports/{MP3_NAME}`",
        f"- Script: `/workspace/build_figment_162.py`",
        f"- Bridge: `C:\\\\Users\\\\Gebruiker\\\\ableton-mcp\\\\evaix-bridge\\\\build_figment_162.py`",
        "",
    ...
    lines += [
        "",
        "## Sound design locks",
        "- Acid: Acid LPF, cutoff muted→open on Drop, res modest, slides+accents, LS13",
        "- Kick: root-tuned F# (~46 Hz sub), drive +EQ-before-sat, sidechain-light pocket",
        "- Drift pad: F#–C#–F#–A# hold, OB LPF character, dark major ≠ happy",
        "- Filters: Acid LPF acid · MG LPF −8va · OB atm — never MS20 scream",
        "- Master: PA DSP tame 3–6 kHz, highshelf −3 dB @7k, loudnorm I=-8.5 TP=-1.0",
        "",
        "## Doctrine kept",
        ...
        "",
        "*EvAIx · Figment Vestiges 01–04 · Architect PRODUCTION GO*",
    ]
...
def main():
    ...
    print("=== EvAIx Figment 162 F# slots 01–04 ===")
    ...
    try:
        shutil.copy2(Path(__file__).resolve(), Path("/workspace/exports/build_figment_162.py"))
    except Exception:
        pass
