# -*- coding: utf-8 -*-
"""EvAIx Tholin Vestiges slots 09–13 @ 162 BPM E major — v2 FULL RENDER (~10').

FineTuniX needs_tweaks fix (cite finetunix-tholin-qc.md):
- Break: FULL kick mute + HPF/duck dual-acid LF → 50–100 Hz collapse; RMS dip >4–6 dB vs Drop_max
- Return harder + sub punch after contrast
- Open LRA (v1 was 2.7 too tight)
KEEP: mid bloom 700Hz–3k, harsh under mids, sample rotation, free-party, dual acid LS13
Default: --skip-ableton (DSP bounce). No Vyre bounce.
"""
...
OUT = Path("/workspace/samples/tholin-162-v2-render") if Path("/workspace").exists() else (
    Path(r"C:\Users\Gebruiker\ableton-mcp\evaix-bridge\samples\tholin-162-v2-render")
)
...
MP3_NAME = "EvAIx_Tholin_162_v2.mp3"
...
NOTES_PATH = (
    Path("/workspace/exports/EvAIx_Tholin_162_v2_NOTES.md")
    if Path("/workspace/exports").exists()
    else OUT / "EvAIx_Tholin_162_v2_NOTES.md"
)
NOTES_WORKSPACE = Path("/workspace/EvAIx_Tholin_162_v2_NOTES.md") if Path("/workspace").exists() else NOTES_PATH
BRIDGE_COPY = Path(r"C:\Users\Gebruiker\ableton-mcp\evaix-bridge\build_tholin_162_v2.py")
MID_JSON = Path("/workspace/exports/_tholin_v2_mid_bloom.json") if Path("/workspace/exports").exists() else OUT / "_tholin_v2_mid_bloom.json"
...
def process_acid_bus(acid: np.ndarray, mg: np.ndarray, open_amt: float, mode: str):
    ...
    # Break: kick-band empty — aggressive HPF so 50–100 Hz collapses (FineTuniX v2)
    if mode == "break_fragile":
        acid = one_pole(acid, 320.0, mode="hpf")
        acid = one_pole(acid, 340.0, mode="hpf")  # steeper
        mg = one_pole(mg, 380.0, mode="hpf")
        mg = one_pole(mg, 400.0, mode="hpf")
        # fragile mid acid only — presence without LF fill
        acid = peaking_eq(acid, 1800.0, 2.2, q=0.95)
        acid = peaking_eq(acid, ACID_PRESENCE_HZ, 2.6, q=1.05)
        acid = peaking_eq(acid, 2800.0, 1.6, q=1.1)
        acid = peaking_eq(acid, 4800.0, -3.2, q=1.15)
        acid = peaking_eq(acid, 80.0, -12.0, q=0.7)   # duck residual kick-band
        acid = peaking_eq(acid, 60.0, -10.0, q=0.8)
        acid = tanh_drive(acid, 1.08)
        acid = one_pole(acid, 4200.0, mode="lpf")
        mg = peaking_eq(mg, 1400.0, 0.8, q=0.9)
        mg = peaking_eq(mg, 70.0, -14.0, q=0.7)
        mg = one_pole(mg, 2800.0, mode="lpf")
        acid *= 0.78
        mg *= 0.55
        return acid, mg
    ...
    if mode in ("drop_open", "drop_max", "return_peak", "return_hard") or o >= 0.65:
        ...
        acid *= 1.20
        mg *= 0.92
    elif mode in ("drop_muted", "drop_prep", "build_muted_acid") or o < 0.40:
...
def kit_for_mode(mode: str, slot: str, shots: dict) -> dict:
    ...
    gains = {
        "intro_glow": dict(kick=0.0, bass=0.0, hats=0.04, oh=0.0, ride=0.0, perc=0.0,
                           acid=0.0, mg=0.0, atm=0.88, pad=0.92, fx=0.35, acid_open=0.10),
        "intro_haze": dict(kick=0.08, bass=0.0, hats=0.06, oh=0.0, ride=0.0, perc=0.0,
                           acid=0.0, mg=0.0, atm=0.80, pad=0.85, fx=0.32, acid_open=0.14),
        ...
        # Break: kick/bass FULL mute; acid+atm mid-fragile only (LF collapsed in process_acid_bus)
        "break_fragile": dict(kick=0.0, bass=0.0, hats=0.0, oh=0.0, ride=0.0, perc=0.0,
                              acid=0.72, mg=0.32, atm=0.70, pad=0.55, fx=0.28, acid_open=0.68),
        "return_peak": dict(kick=1.28, bass=1.22, hats=0.58, oh=0.35, ride=0.92, perc=0.88,
                            acid=1.18, mg=0.72, atm=0.08, pad=0.18, fx=0.30, acid_open=0.90),
        "return_hard": dict(kick=1.36, bass=1.30, hats=0.62, oh=0.40, ride=0.98, perc=0.95,
                            acid=1.22, mg=0.75, atm=0.06, pad=0.14, fx=0.32, acid_open=0.92),
    }
    stories = {
        ...
        "break_fragile": "Slot12 Break — Kick OUT + acid LF HPF/duck; fragile mid acid+atm only",
        "return_peak": "Slot13 Return — harder kick + sub punch after Break contrast",
        "return_hard": "Slot13 Return — hardest E08+bass-kick + sub; handoff peel toward B",
    }
    kick_on = {
...
def render_section(idx: int, name: str, bars: int, mode: str, slot: str, shots: dict):
    ...
    pad_open = {
        ...
        "drop_max": 0.26, "break_fragile": 0.42, "return_peak": 0.32, "return_hard": 0.28,
    }[mode]
    if g["pad"] > 0.01:
    ...
    # Return: extra sub punch after Break contrast
    if mode in ("return_peak", "return_hard"):
        trk_k = peaking_eq(trk_k, 41.2, 2.8 if mode == "return_hard" else 2.2, q=1.0)
        trk_k = peaking_eq(trk_k, 110.0, 2.0 if mode == "return_hard" else 1.6, q=0.9)
        trk_b = peaking_eq(trk_b, 48.0, 2.0, q=0.9)
        trk_k = tanh_drive(trk_k, 1.18 if mode == "return_hard" else 1.12)

    if g["acid"] > 0.05 or g["mg"] > 0.05:
        trk_acid, trk_mg = process_acid_bus(trk_acid, trk_mg, acid_open, mode)

    # Break: strip residual kick-band from pad/atm (pad fund is E2 ~82 Hz)
    if mode == "break_fragile":
        trk_pad = one_pole(trk_pad, 220.0, mode="hpf")
        trk_pad = one_pole(trk_pad, 240.0, mode="hpf")
        trk_pad = peaking_eq(trk_pad, 70.0, -14.0, q=0.7)
        trk_atm = one_pole(trk_atm, 200.0, mode="hpf")
        trk_fx = one_pole(trk_fx, 180.0, mode="hpf")
        # hard assert: no kick/bass energy
        trk_k *= 0.0
        trk_b *= 0.0
    ...
    if mode == "break_fragile":
        ...
        acid_bus = one_pole(acid_bus, 300.0, mode="hpf")
        acid_bus = peaking_eq(acid_bus, 75.0, -10.0, q=0.7)
        hats_atm = one_pole(hats_atm, 180.0, mode="hpf")
    ...
    if mode in ("drop_open", "drop_max", "return_hard"):
        ...
        acid_bus = apply_moving_lpf(acid_bus, 2400.0, 5800.0)
        acid_bus = one_pole(acid_bus, 310.0, mode="hpf")
    else:
...
def bounce_mp3(mixes, mp3_path: Path) -> float:
    ...
    af = (
        ...
        "lowshelf=f=85:t=q:w=0.7:g=1.5,"
        "equalizer=f=41:t=q:w=1.1:g=2.0,"
        ...
        # softer glue so Intro→Break→Return LRA can open (v1 LRA 2.7)
        "acompressor=threshold=-20dB:ratio=1.28:attack=12:release=180:makeup=1.05,"
    ...
    # Surgical: Drop_open mid bloom 280–339s + Break LF collapse / level dip 420.7–480s
    # + Return sub punch 480–600s
    af2 = (
        ...
        "highpass=f=220:poles=2:enable='between(t,420.7,480)',"
        "equalizer=f=60:t=q:w=0.8:g=-14:enable='between(t,420.7,480)',"
        "equalizer=f=80:t=q:w=0.9:g=-12:enable='between(t,420.7,480)',"
        "equalizer=f=100:t=q:w=1.0:g=-8:enable='between(t,420.7,480)',"
        "volume=volume=-5.5dB:enable='between(t,420.7,480)',"
        "equalizer=f=41:t=q:w=1.0:g=2.5:enable='between(t,480,600)',"
        "equalizer=f=110:t=q:w=0.9:g=1.8:enable='between(t,480,600)',"
        "loudnorm=I=-8.5:TP=-1.0:LRA=11,"
        "alimiter=limit=0.891:attack=1:release=50:level=disabled"
    )
    r2 = subprocess.run(
...
def measure_break_vs_drop(path: Path) -> dict:
    """FineTuniX v2: Break RMS dip vs Drop_max + 50–100 Hz / kick-band collapse."""
    raw = OUT / "_tholin_v2_break_analyze.f32"
    subprocess.run(
        [FFMPEG, "-y", "-i", str(path), "-ac", "1", "-ar", "48000", "-f", "f32le", str(raw)],
        check=True, capture_output=True,
    )
    ...
    # Timeline from NOTES: Drop_max 380.7–420.7 · Break 420.7–480 · Return 480–600
    drop = section(390.0, 415.0)   # solid Drop_max body (avoid last-8 kick mute)
    brk = section(430.0, 470.0)    # mid Break
    ret = section(490.0, 530.0)    # Return_a body

    drop_rms = rms_db(drop)
    brk_rms = rms_db(brk)
    ret_rms = rms_db(ret)
    dip = brk_rms - drop_rms
    ret_vs = ret_rms - brk_rms

    drop_lf = band_db(drop, 50.0, 100.0)
    brk_lf = band_db(brk, 50.0, 100.0)
    lf_delta = brk_lf - drop_lf

    def kickband_rel(seg):
        full = band_db(seg, 20.0, 12000.0)
        kb = band_db(seg, 40.0, 100.0)
        return kb - full

    drop_kb = kickband_rel(drop)
    brk_kb = kickband_rel(brk)
    out = {
        "drop_max_rms_db": round(drop_rms, 2),
        "break_rms_db": round(brk_rms, 2),
        "return_rms_db": round(ret_rms, 2),
        "break_vs_drop_rms_db": round(dip, 2),
        "return_vs_break_rms_db": round(ret_vs, 2),
        "drop_max_50_100_db": round(drop_lf, 2),
        "break_50_100_db": round(brk_lf, 2),
        "lf_50_100_delta_db": round(lf_delta, 2),
        "drop_max_kickband_rel_db": round(drop_kb, 2),
        "break_kickband_rel_db": round(brk_kb, 2),
        "kickband_rel_delta_db": round(brk_kb - drop_kb, 2),
        "rms_dip_pass": bool(dip <= -4.0),
        "lf_collapse_pass": bool(lf_delta <= -6.0),
    }
    print("BREAK-METRICS", out)
    jpath = Path("/workspace/exports/_tholin_v2_break_metrics.json")
    if Path("/workspace/exports").exists():
        jpath.write_text(json.dumps(out, indent=2), encoding="utf-8")
    (OUT / "_tholin_v2_break_metrics.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
    return out


def write_notes(dur: float, metas: list, lufs: dict, shots: dict, errors: list,
                mid_delta: dict | None = None, break_metrics: dict | None = None):
    t0 = 0.0
    lines = [
        "# EvAIx Tholin v2 — Vestiges slots 09–13 @ 162 E (FineTuniX Break fix)",
        "",
        f"**MP3:** `{MP3_NAME}`",
        f"- Downloads: `C:\\\\Users\\\\Gebruiker\\\\Downloads\\\\{MP3_NAME}`",
        f"- Workspace: `/workspace/{MP3_NAME}`",
        f"- Exports: `/workspace/exports/{MP3_NAME}`",
        f"- Script: `/workspace/build_tholin_162_v2.py`",
        "- QC cite: `/workspace/finetunix-tholin-qc.md` (needs_tweaks → Break LF + LRA)",
    ...
    if break_metrics:
        lines += [
            "## Break vs Drop_max (FineTuniX v2 gates — cite finetunix-tholin-qc.md)",
            "| Metric | Value | Target |",
            "|--------|-------|--------|",
            f"| Drop_max RMS | **{break_metrics.get('drop_max_rms_db')}** dBFS | — |",
            f"| Break RMS | **{break_metrics.get('break_rms_db')}** dBFS | — |",
            f"| Break − Drop_max RMS | **{break_metrics.get('break_vs_drop_rms_db')}** dB | dip **>4–6 dB** |",
            f"| Drop_max 50–100 Hz | **{break_metrics.get('drop_max_50_100_db')}** dB | — |",
            f"| Break 50–100 Hz | **{break_metrics.get('break_50_100_db')}** dB | collapse |",
            f"| 50–100 Hz delta (Break−Drop_max) | **{break_metrics.get('lf_50_100_delta_db')}** dB | deep hole |",
            f"| Drop_max kick-band 40–100 rel | **{break_metrics.get('drop_max_kickband_rel_db')}** dB | — |",
            f"| Break kick-band 40–100 rel | **{break_metrics.get('break_kickband_rel_db')}** dB | empty |",
            f"| Kick-band rel delta | **{break_metrics.get('kickband_rel_delta_db')}** dB | clear collapse |",
            f"| Return − Break RMS | **{break_metrics.get('return_vs_break_rms_db')}** dB | punch after contrast |",
            f"| Gates | RMS dip pass=**{break_metrics.get('rms_dip_pass')}** · LF collapse pass=**{break_metrics.get('lf_collapse_pass')}** | |",
            "",
            "v2 fix applied: Break full kick mute + dual-acid HPF≈320 Hz + pad/atm HPF + surgical master HPF/duck 420.7–480s (−5.5 dB); Return kick/bass up + sub EQ 41/110 Hz.",
            "",
        ]
    lines += [
        ...
        "| 12 | Tholin_Break | 7:00–8:00 | 4 | Kick OUT · acid LF HPF/duck · fragile mid acid+atm |",
        "| 13 | Tholin_Return | 8:00–10:00 | 9 | Harder kick + sub punch after Break contrast |",
        "",
    ...
    lines += [
        ...
        "- Break acid: HPF≈320 Hz + 50–100 Hz duck; pad/atm HPF≈200 Hz; kick/bass=0",
        "- Kick: root-tuned E (~41.2 Hz sub), punch body ~110 Hz, EQ before sat, mono",
        "- Return: E07/E08 + KICK BASS E; extra sub EQ 41/110 Hz after Break contrast",
        "- Perc: Pneumatix tribal UP · poly LS7 conga/bongo/djembe",
        "- Master: mid lift + PA tame; Break surgical HPF/duck 420.7–480s; loudnorm I=-8.5 TP=-1.0 LRA=11",
        "",
        "## Doctrine kept",
        ...
        "",
        "*EvAIx · Tholin v2 Vestiges 09–13 · FineTuniX Break LF + LRA fix · peak chapter*",
        "",
        "*FineTuniX gates: mid bloom kept · Break RMS/LF measured above · I/TP in band · free-party*",
    ]
    text = "\n".join(lines)
...
def render_full() -> int:
    print("=== EvAIx Tholin 162 v2 · Break LF collapse · Return punch · LRA open ===")
    ...
    if lufs.get("I") is not None and (lufs["I"] < -9.5 or lufs["I"] > -7.5):
        ...
        af = "loudnorm=I=-8.5:TP=-1.0:LRA=11,alimiter=limit=0.891:attack=1:release=50:level=disabled"
    ...
    break_metrics = measure_break_vs_drop(MP3_WORKSPACE)
    write_notes(dur, metas, lufs, shots, errors, mid_delta=mid_delta, break_metrics=break_metrics)
    ...
    try:
        shutil.copy2(Path(__file__).resolve(), Path("/workspace/exports/build_tholin_162_v2.py"))
    except Exception:
        pass
    ...
    print("DONE", MP3_WORKSPACE, "I=", lufs.get("I"), "TP=", lufs.get("TP"),
          ...
          "break_dip=", break_metrics.get("break_vs_drop_rms_db"),
          "lf_delta=", break_metrics.get("lf_50_100_delta_db"),
          "rms_pass=", break_metrics.get("rms_dip_pass"),
          "lf_pass=", break_metrics.get("lf_collapse_pass"))
    if errors:
        print("ERRORS", errors)
    return 0
